...
- [x] **Task 4: Implement reconnect + warm-up in agent** — In `server/ambient-agent/agent.ts`, in `handleWatchStatus()` when `connected === true`:
...
- [x] **Task 5: Add warm-up disqualifier + backfill filter** — In `server/ambient-agent/agent.ts`:
...
- [x] **Task 6: Write gap handling tests** — Create `server/ambient-agent/agent-gap-handling.test.ts` following the pattern of `agent-location-staleness.test.ts` (use `(agent as any)` for internals, fake relay URL `ws://localhost:9999/browser`, no real WebSocket). Tests:
...
- [x] **Task 7: Update STANDARDS.md** — Append to existing `STANDARDS.md` (do NOT overwrite):
