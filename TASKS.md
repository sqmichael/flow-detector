...
- [ ] Add `location_lat`, `location_lon`, `location_accuracy` columns to `watch_batches` table in `server/sensor-fusion/database.ts` (ALTER TABLE migration + CREATE TABLE update). Add matching fields to `WatchBatchRow` and `WatchBatchInsert` in `server/sensor-fusion/types.ts`. Verify with `npx tsc --noEmit`.
