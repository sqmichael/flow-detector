...
- [x] Update `agent.ts` to call `buildDynamicContext()` before each intervention decision in both `processViaOpenClaw()` and the reasoning.ts fallback path. Pass context through to both.
